...
- **인증**: 쿠키 기반 mock 세션. `/api/auth/signup`, `/api/auth/login`, `/api/auth/logout`. 세션 쿠키가 있으면 `auth-gate`가 통과시킨다.
...
- `provisioner`는 `eval-spec.preset` 값이 `booking-web`이면 `presets/booking-web/core/scaffold/`에서 workspace를 생성한다. 이는 이미 "preset 폴더를 그대로 복사"하는 구조라 추가 코드가 필요 없다.
- `agents/builder.md`, `agents/test-writer.md`는 preset의 `rules/CLAUDE.md`와 `test-pack/`을 그대로 읽으므로 수정 불필요.
- evaluator 5종(`build`, `unit_test`, `page_check`, `console`, `e2e`)은 전부 그대로 재사용.
...
1. **scaffold 생성** — admin-web scaffold를 기준으로 fork 후 (public)/(member) 라우트·레이아웃·카탈로그 카드/그리드·auth-gate 구현. `npm run build` 통과.
2. **rules/CLAUDE.md + protected-files.json 작성** — §8 기준.
3. **test-pack 템플릿 8개 작성** — §7 기준. 각 템플릿은 admin-web 템플릿 스타일을 따른다(플레이스홀더 치환 변수, Playwright 패턴).
4. **examples/resort-booking-spec.yaml 확정본 작성** — §10 기준.
5. **스모크 실행** — `/fde-agent:run examples/resort-booking-spec.yaml`을 반복하며 evaluator 리포트가 pass에 도달할 때까지 preset scaffold/rules/test-pack을 조정.
6. **README 업데이트** — preset 표에 booking-web 추가, Phase 4 로드맵 상태 업데이트.
